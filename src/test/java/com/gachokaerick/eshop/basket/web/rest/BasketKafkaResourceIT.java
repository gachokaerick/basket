...
import com.gachokaerick.eshop.basket.config.KafkaProperties;
...
class BasketKafkaResourceIT {
    ...
    void setup() {
        ...
        BasketKafkaResource kafkaResource = new BasketKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/basket-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/basket-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
